...
pub fn (app &Role) find_tenant_role_all_handler(mut ctx Context) veb.Result {
	...
	result := find_tenant_role_all_usecase(mut ctx, req) or {
...
pub fn find_tenant_role_all_usecase(mut ctx Context, req GetTenantRoleListReq) !GetTenantRoleListResp {
	// Domain 参数校验
	validate_tenant_role_list_domain(req)!

	// Repository 查询
	return find_tenant_role_all_repo(mut ctx, req)
}
...
// ----------------- Repository 层 -----------------
fn find_tenant_role_all_repo(mut ctx Context, req GetTenantRoleListReq) !GetTenantRoleListResp {
