...
pub fn (app &User) find_user_info_handler(mut ctx Context) veb.Result {
	...
	result := find_user_info_usecase(mut ctx, req) or {
...
pub fn find_user_info_usecase(mut ctx Context, req GetUserInfoReq) !GetUserInfoResp {
	// Domain 层参数校验
	find_user_info_domain(req)!

	// Repository 查询数据库
	return find_user_info_repo(mut ctx, req)
}

// ----------------- Domain 层 -----------------
fn find_user_info_domain(req GetUserInfoReq) ! {
	if req.user_id == '' {
...
// ----------------- Repository 层 -----------------
fn find_user_info_repo(mut ctx Context, req GetUserInfoReq) !GetUserInfoResp {
