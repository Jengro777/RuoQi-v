...
pub fn (app &Menu) find_menu_all_by_role_handler(mut ctx Context) veb.Result {
	...
	result := find_menu_all_by_role_usecase(mut ctx) or {
...
// ----------------- Usecase 层 -----------------
pub fn find_menu_all_by_role_usecase(mut ctx Context) !RoleMenuListResp {
	role_ids := ctx.svc_sys.role_ids
	find_menu_all_by_role_domain(role_ids)!

	return find_menuids_by_roleids(mut ctx, role_ids)
}

// ----------------- Domain 层 -----------------
fn find_menu_all_by_role_domain(roleids []string) ! {
...
fn find_menuids_by_roleids(mut ctx Context, roleids []string) !RoleMenuListResp {
	...
	if roleids == ['*'] {
		...
		// ------------------- 查询菜单信息 -------------------
		result = sql db {
			select from SysMenu where id in menu_ids order by sort
		} or { return error('Failed to query menu information: ${err}') }
	}

	result.sort(a.sort < b.sort)
	...
	for row in result {
		datalist << MenuDataList{
			...
			meta:         MenuMeta{
				...
				title:                 ctx.i18n.t(row.title.str())
			}
			path:         row.path or { '' }
			name:         row.name.str()
