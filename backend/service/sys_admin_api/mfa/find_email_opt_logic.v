...
// ----------------- Handler 层 -----------------
@['/login_by_email'; post]
pub fn (app &MFA) find_email_opt_handler(mut ctx Context) veb.Result {
	...
	result := find_email_opt_usecase(req) or {
...
// ----------------- Usecase 层 -----------------
pub fn find_email_opt_usecase(req EmailLoginReq) !EmailLoginResp {
	// Domain 参数校验
	find_email_opt_domain(req)!

	// Repository 层生成 OTP
	return find_email_opt_repo()
}

// ----------------- Domain 层 -----------------
fn find_email_opt_domain(req EmailLoginReq) ! {
	if req.email == '' {
...
fn find_email_opt_repo() !EmailLoginResp {
	log.debug('${@METHOD}  ${@MOD}.${@FILE_LINE}')
