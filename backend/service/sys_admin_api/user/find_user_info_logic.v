...
pub fn find_user_info_usecase(mut ctx Context) !GetUserInfoResp {
	// 调用 Domain 层校验
	find_user_info_domain()!

	user_id := find_userid_by_token(mut ctx)!
	// 调用 Repository 层获取用户信息
	return find_user_info(mut ctx, user_id)!
}

// ----------------- Domain 层 -----------------
fn find_user_info_domain() ! {
	// if req.user_id == '' {
...
fn find_user_info(mut ctx Context, user_id string) !GetUserInfoResp {
