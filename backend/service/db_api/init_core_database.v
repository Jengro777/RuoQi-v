...
pub fn (app &Base) init_core(mut ctx Context) veb.Result {
	...
	match ctx.config.dbconf.type {
		'mysql' {
			mut p := ctx.dbpool as &dbpool.DatabasePool[mysql.DB]
			db, conn := p.acquire_raw() or {
				return ctx.json(api.json_error_500('获取的连接无效: ${err}'))
			}
			defer {
				p.release(conn) or { log.warn('Failed to release connection ${@LOCATION}: ${err}') }
			}
			...
			sql_commands := [core_api, core_app, core_app_client, core_connector, core_menu,
				core_project, core_role, core_role_api, core_role_menu, core_role_tenant_member,
				core_tenant, core_tenant_member, core_tenant_subapp, core_token, core_user,
				core_user_connector]
			...
			return ctx.json(api.json_error_500('Unsupported db_type: ${ctx.config.dbconf.type}'))
		}
	}
}
