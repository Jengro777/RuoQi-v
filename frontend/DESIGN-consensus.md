...
| `row-actions-menu`   | `RuQiRowActionsMenu`                                         | 表格「操作」列：动作 **≥3** 时统一收进 `···` 菜单（与 用户 页一致），≤2 时保持行内文字按钮；`···` 命中区 36×36，悬停只把图标转 `primary`；菜单面板白底 + 1px `outlineVariant` 描边，菜单项 36 高、悬停文案转 `primary`（无灰底） |
